...
class WarmupService:
    ...
    async def get_completed_warmups_in_period(self, start_date: datetime, end_date: datetime) -> List[UserWarmup]:
        """Получить завершенные прогревы за период."""
        try:
            stmt = (
                select(UserWarmup)
                .where(
                    and_(
                        UserWarmup.is_completed == True,
                        UserWarmup.updated_at >= start_date,
                        UserWarmup.updated_at <= end_date
                    )
                )
            )
            result = await self.session.execute(stmt)
            ...
            logger.error(f"Ошибка получения завершенных прогрева за период: {e}")
            return []
    
    async def get_sent_messages_in_period(self, start_date: datetime, end_date: datetime, message_type: str = None) -> int:
        """Получить количество отправленных сообщений прогрева за период."""
        try:
            from app.models.user_warmup_message import UserWarmupMessage
            
            query = select(UserWarmupMessage).where(
                and_(
                    UserWarmupMessage.sent_at >= start_date,
                    UserWarmupMessage.sent_at <= end_date,
                    UserWarmupMessage.is_sent == True
                )
            )
            
            if message_type:
                # Подзапрос для получения ID сообщений нужного типа
                message_ids = select(WarmupMessage.id).where(WarmupMessage.message_type == message_type)
                query = query.where(UserWarmupMessage.warmup_message_id.in_(message_ids))
            
            result = await self.session.execute(query)
            return len(result.scalars().all())
        except Exception as e:
            logger.error(f"Ошибка получения отправленных сообщений за период: {e}")
            return 0
